...
// The Thursday payout report, emailed by the backend.
//
//   npm test
//
// This replaced a scheduled task on Jonathan's own computer, which only ran if
// the laptop happened to be awake at 8am on a Thursday. He drives a truck. A
...
// Thursdays at 8am in New York, one inside daylight saving and one outside.
const THU_AUG = new Date('2026-08-20T12:00:00Z');   // 08:00 EDT
const THU_DEC = new Date('2026-12-17T13:00:00Z');   // 08:00 EST

async function main() {
  console.log('\nKnowing what day it is where the creators are');
  check('summer: noon UTC is 8am Thursday in New York',
    localNow(THU_AUG, 'America/New_York'), { date: '2026-08-20', hour: 8, dow: 4 });
  ...
  check('winter: it takes 1pm UTC to be 8am Thursday',
    localNow(THU_DEC, 'America/New_York'), { date: '2026-12-17', hour: 8, dow: 4 });
  check('and noon UTC in December is only 7am',
    localNow(new Date('2026-12-17T12:00:00Z'), 'America/New_York').hour, 7);

  console.log('\nWhen it fires');
  check('not on a Wednesday',
    await maybeSendPayoutReport(new Date('2026-08-19T12:00:00Z')), 'not now');
  check('not at 9am on the Thursday',
    await maybeSendPayoutReport(new Date('2026-08-20T13:00:00Z')), 'not now');
  check('nothing has been emailed yet', sent.length, 0);

  console.log('\nThursday, 8am');
  check('it sends', await maybeSendPayoutReport(THU_AUG), 'sent');
  ...
  check('the second run stands down', await maybeSendPayoutReport(THU_AUG), 'already sent');
  check('still one email', sent.length, 1);
  const laterSameHour = new Date('2026-08-20T12:59:00Z');
  check('and so does one later in the same hour',
    await maybeSendPayoutReport(laterSameHour), 'already sent');
  check('still one email', sent.length, 1);

  console.log('\nNext week');
  check('a new Thursday sends again',
    await maybeSendPayoutReport(new Date('2026-08-27T12:00:00Z')), 'sent');
  ...
  check('the failure is reported, not swallowed',
    await maybeSendPayoutReport(THU_DEC), 'failed');
